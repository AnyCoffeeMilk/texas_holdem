...
export default function OnlineGame({ roomId }) {
   ...
   const gamerB = useOnlineGamer()
   const gamerC = useOnlineGamer()
   ...
   const {
      ...
      newRound,
      addPlayer,
   } = useOnlineTurnHandler(gameTable)
   ...
   useEffect(() => {
      ...
      updatePlayersChannel.bind(roomId, (data) => {
         const new_opponents = data.playerList.filter(
            (item) =>
               !opponentList
                  .map((opponent) => opponent.uuid)
                  .includes(item.uuid) && item.uuid !== player.uuid
         )
   ...
   return !player.avatar ? null : (
      <div className="grid min-h-[calc(100svh-1rem)] max-w-[480px] min-w-[350px] grid-cols-1 grid-rows-[auto_auto_1fr_auto] gap-2 sm:w-[620px] sm:max-w-none sm:min-w-auto lg:w-[1024px] lg:grid-cols-[auto_1fr] lg:grid-rows-[auto_1fr_auto] lg:gap-4 lg:p-4">
         ...
         <div className="container-sm sm:container-md flex-center relative col-1 lg:col-2 row-3 rounded-sm px-2 sm:px-4 lg:row-2">
