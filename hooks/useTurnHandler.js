    ...
    switch (gameStateId) {
      case 0: // BB and SB Set Bets State
        const new_smallBlind =
          smallBlind !== -1 ? (smallBlind < 3 ? smallBlind + 1 : 0) : Math.floor(Math.random() * 4);
        ...
        if (new_smallBlind === 0) {
          ...
          add_bank(-2).then((new_bank) => gamers_list[0].setBank(new_bank));
        }
        ...
        if (new_bigBlind === 0) {
          // TODO, now only reduce the bank of player
          add_bank(-4).then((new_bank) => gamers_list[0].setBank(new_bank));
        }
      ...
      case 5: // Check Winner State
        const winner_id = getWinner(
          turnQueue.map((id) => ({ name: id, cards: gamers_list[id].cards })),
          gameTable.cards
        );
        let total_bets = 0;
        for (let i = 0; i < 4; i++) {
        ...
        if (winner_id.length > 1) {
          ...
          if (winner_id === 0) {
            ...
            add_bank(total_bets / winner_id.length).then((new_bank) => gamers_list[0].setBank(new_bank));
          }
        } else {
          ...
          if (winner_id === 0) {
            // TODO, now only add to the bank of player
            add_bank(total_bets).then((new_bank) => gamers_list[0].setBank(new_bank));
          }
        }
        break;
    }
